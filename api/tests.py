...
class ChatTest(APITestCase):
    ...
    def setUp(self):
        print('создаю данные')
        self.user1 = User.objects.create(username='user_1',password='gd_gdferw_ew')
        Token.objects.create(user=self.user1)
        self.user2 = User.objects.create(username='user_2',password='gd_gdferw_ew')
        Token.objects.create(user=self.user2)
        self.user3 = User.objects.create(username='user_3',password='gd_gdferw_ew')
    ...
    def test_chat1_change_name(self):
        request = self.create_request('chat_room/',self.user1,'PUT',data={'name':'new_name'})
        data = chat_room(request,self.chat1.id).data
        self.assertEquals(data['name'], 'new_name')

    def test_create_chat(self):
        request = self.create_request('add_chat_room/',self.user1,'POST',data={'name':'new_chat'})
    ...
    def test_add_user_to_chat(self):
        request = self.create_request('add_user_to_chat_room/',self.user1,'POST',data={'chat_room':'2','user':'2'})
        data = add_user_to_chat_room(request).data
        check_user_in_chat = Chat_room_users.objects.filter(chat_room = self.chat2,user = self.user2).exists()
        self.assertEquals(check_user_in_chat, True)



    def test_secure_add_user_to_chat(self):
        request = self.create_request('add_user_to_chat_room/',self.user2,'POST',data={'chat_room':'2','user':'3'})
        data = add_user_to_chat_room(request).data
        check_user_in_chat = Chat_room_users.objects.filter(chat_room = self.chat2,user = self.user3).exists()
